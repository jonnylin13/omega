...
import { MapleAESOFB } from '../../../src/net/crypto/aes';
...
describe('MapleAESOFB test', () => {
    it('test multiply_bytes()', () => {
        let result = Compare.compare_buffers(MapleAESOFB.multiply_bytes(input, 4, 4), expected_output);
    ...
    it('test get_new_iv()', () => {
        let result = Compare.compare_buffers(MapleAESOFB.get_new_iv(Buffer.from([0x01, 0x02, 0x03, 0x80])), Buffer.from([-102, -117, -63, 34]));
        expect(result).equal(true);
    });
